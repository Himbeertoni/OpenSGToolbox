...
// OSGTBWIN32WindowBase.cpp file.
// To modify it, please change the .fcd file (OSGTBWIN32Window.fcd) and
...
TBWIN32Window::TBWIN32HWNDToProducerMap TBWIN32Window::_WIN32HWNDToProducerMap;

/***************************************************************************\
 *                           Class methods                                 *
\***************************************************************************/

void TBWIN32Window::initMethod(InitPhase ePhase)
{
...
void  TBWIN32Window::mainLoop(void)
{
    // Main loop ( event dispatching )
    MSG msg;
    // main loop 
    TBWIN32HWNDToProducerMap::iterator MapItor;
...
std::string TBWIN32Window::getClipboard(void) const
{
...
void TBWIN32Window::putClipboard(const std::string Value)
{
    if(OpenClipboard(getHwnd()))
    {
        if(EmptyClipboard())
...
void TBWIN32Window::setCursor(void)
{
    LPSTR c;
    switch(getCursorType())
    {
	    case WindowEventProducer::CURSOR_POINTER:
            c = IDC_ARROW;
            break;
        case WindowEventProducer::CURSOR_HAND:
            c = IDC_HAND;
            break;
        case WindowEventProducer::CURSOR_I_BEAM:
            c = IDC_IBEAM;
            break;
        case WindowEventProducer::CURSOR_WAIT:
            c = IDC_WAIT;
            break;
        case WindowEventProducer::CURSOR_RESIZE_W_TO_E:
            c = IDC_SIZEWE;
            break;
        case WindowEventProducer::CURSOR_RESIZE_N_TO_S:
            c = IDC_SIZENS;
            break;
        case WindowEventProducer::CURSOR_RESIZE_NW_TO_SE:
            c = IDC_SIZENWSE;
            break;
        case WindowEventProducer::CURSOR_RESIZE_SW_TO_NE:
            c = IDC_SIZENESW;
            break;
        case WindowEventProducer::CURSOR_RESIZE_ALL:
            c = IDC_SIZEALL;
            break;
        case WindowEventProducer::CURSOR_NONE:
            c = IDC_NO;
            break;
        default:
            c = IDC_ARROW;
...
Pnt2f TBWIN32Window::getMousePosition(void) const
{
...
UInt32 TBWIN32Window::getKeyModifiers(void) const
{
    UInt32 Modifiers = 0;
...
KeyEventDetails::KeyState TBWIN32Window::getKeyState(KeyEventDetails::Key TheKey) const
{
    SHORT KeyState(0);
    switch(TheKey)
    {
...
KeyEventDetails::Key TBWIN32Window::determineKey(WPARAM key)
{
...
BoostPath TBWIN32Window::saveFileDialog(const std::string& DialogTitle,
                ...
                )
{
    ...
    UInt32 FilterSize(0);
    for(std::vector<WindowEventProducer::FileDialogFilter>::const_iterator Itor(Filters.begin()) ; Itor != Filters.end(); ++Itor)
    {
...
std::vector<BoostPath> TBWIN32Window::openFileDialog(const std::string& WindowTitle,
		const std::vector<WindowEventProducer::FileDialogFilter>& Filters,
...
Vec2f TBWIN32Window::getDesktopSize(void) const
{
    DEVMODE dm;
...
void TBWIN32Window::draw(void)
{
    //SendMessage(getHwnd(),WIN32_DRAW_MESSAGE,0,0);
    internalDraw();
}

void TBWIN32Window::update(void)
{
...
Window* TBWIN32Window::initWindow(void)
{
	...
	wndClass.lpfnWndProc		= (WNDPROC) TBWIN32Window::staticWndProc;				// WndProc Handles Messages
...
void TBWIN32Window::openWindow(const Pnt2f& ScreenPosition,
                    const Vec2f& Size,
...
bool TBWIN32Window::attachWindow(void)
{
...
LRESULT TBWIN32Window::WndProc(HWND hwnd, UINT uMsg,
                     WPARAM wParam, LPARAM lParam)
{
    switch(uMsg)
    {       
...
void TBWIN32Window::setShowCursor(bool showCursor)
{
...
bool TBWIN32Window::getShowCursor(void) const
{
    ...
    {
        SWARNING << "TBWIN32Window::getShowCursor: Failed to get cursor info " << GetLastError() << std::endl;
    }
    return TheCursorInfo.flags == CURSOR_SHOWING;
}

void TBWIN32Window::setAttachMouseToCursor(bool attachCursor)
{
    _IsMouseCursorAssociated = attachCursor;
}

bool TBWIN32Window::getAttachMouseToCursor(void) const
{
    return _IsMouseCursorAssociated;
}

void TBWIN32Window::setCursorPos(Vec2f Pos)
{
    if(!SetCursorPos(static_cast<int>(Pos.x()), static_cast<int>(Pos.y())))
    {
        SWARNING << "TBWIN32Window::setCursorPos: Failed to set cursor position" << std::endl;
    }
}

//Set the Window Position
void TBWIN32Window::setPosition(Pnt2f Pos)
{
...
//Set the Window Position
Pnt2f TBWIN32Window::getPosition(void) const
{
    RECT r;
...
//Set the Window size
void TBWIN32Window::setSize(Vec2us Size)
{
...
//Get the Window size
Vec2f TBWIN32Window::getSize(void) const
{
    RECT r;
    GetWindowRect(getHwnd(), &r);
...
void TBWIN32Window::setFocused(bool Focused)
{
    if(Focused)
    {
        SetFocus(getHwnd());
...
bool TBWIN32Window::getFocused(void) const
{
...
void TBWIN32Window::setVisible(bool Visible)
{
    if(Visible)
    {
...
bool TBWIN32Window::getVisible(void) const
{
   return static_cast<bool>(IsWindowVisible(getHwnd()));
}

//Set the Window Iconify
void TBWIN32Window::setIconify(bool Iconify)
{
    if(Iconify)
    {
...
bool TBWIN32Window::getIconify(void) const
{
   return static_cast<bool>(IsIconic(getHwnd()));
}

//Fullscreen
void TBWIN32Window::setFullscreen(bool Fullscreen)
{
...
bool TBWIN32Window::getFullscreen(void) const
{
    return _IsFullscreen;
}

void TBWIN32Window::closeWindow(void)
{
    SendMessage(getHwnd(),WM_CLOSE,0,0);
}

void TBWIN32Window::setTitle(const std::string& TitleText)
{
    SetWindowText(getHwnd(), TitleText.c_str());
}

std::string TBWIN32Window::getTitle(void)
{
...
void TBWIN32Window::setDrawBorder(bool DrawBorder)
{
    //TODO:Implement
}

bool TBWIN32Window::getDrawBorder(void)
{
    //TODO:Implement
    return true;
}
 ...
 *                           Instance methods                              *
\***************************************************************************/
void TBWIN32Window::onCreate(const TBWIN32Window *source)
{
...
void TBWIN32Window::onDestroy(UInt32 uiContainerId)
{
...
TBWIN32Window::TBWIN32Window(const TBWIN32Window &source) :
    Inherited(source),
    _MouseOverWindow(false),
    _IsFullscreen(false),
    _IsMouseCursorAssociated(true),
...
void TBWIN32Window::changed(ConstFieldMaskArg whichField, 
                            UInt32            origin,
                            BitVector         details)
{
...
void TBWIN32Window::dump(      UInt32    ,
                         const BitVector ) const
{
    SLOG << "Dump TBWIN32Window NI" << std::endl;
}
